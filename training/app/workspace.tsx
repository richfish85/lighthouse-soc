...
export default function Home() {
  ...
  const attempt: Attempt = progress[c.id] ?? { answers: {}, notes: "", submitted: false, tries: 0 };
  ...
  function submit() {
    ...
    if (attempt.notes.trim().length < 40) { setFormError("Add a handover of at least 40 characters to your notebook. It is for self-review, not automatically graded."); return; }
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      <button className="brand" onClick={() => setView("path")}><span className="brand-icon"><Shield size={24}/></span><span>LIGHTHOUSE<span className="brand-sub">SOC TRAINING</span></span></button>
      ...
      <div className="sidebar-bottom"><span className="local-dot"/> Practice environment<p>Fictional cases. Real reasoning.</p><a href="https://github.com/richfish85/lighthouse-soc" target="_blank" rel="noreferrer">View project on GitHub ↗</a></div>
    </aside>
    <div className="workspace">
      <header className="topbar"><span>Training workspace <ChevronRight size={14}/> {view === "case" ? c.id : view === "progress" ? "My progress" : view === "guide" ? "Field guide" : "Learning path"}</span><span className="demo-tag"><FlaskConical size={15}/> SYNTHETIC LAB</span></header>
